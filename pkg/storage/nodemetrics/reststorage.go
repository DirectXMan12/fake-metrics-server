...
func NewStorage(groupResource schema.GroupResource, metricsProvider provider.MetricsProvider, nodeLister v1listers.NodeLister) *MetricStorage {
...
func (m *MetricStorage) List(ctx genericapirequest.Context, options *metainternalversion.ListOptions) (runtime.Object, error) {
	...
	for _, node := range nodes {
		if m := m.getNodeMetrics(node); m != nil {
...
func (m *MetricStorage) Get(ctx genericapirequest.Context, name string, opts *metav1.GetOptions) (runtime.Object, error) {
	...
	// TODO: actually pass the real node here
	nodeMetrics := m.getNodeMetrics(&v1.Node{ObjectMeta: metav1.ObjectMeta{Name: name}})
	if nodeMetrics == nil {
...
func (m *MetricStorage) getNodeMetrics(node *v1.Node) *metrics.NodeMetrics {
	info, found := m.metricsProvider.ForNode(node)
	if !found {
		glog.Errorf("No metrics for node %s", node.Name)
		return nil
	}
	...
	return &metrics.NodeMetrics{
		...
		Usage:     metrics.ResourceList{
			metrics.ResourceName(v1.ResourceCPU.String()): *info.CPU(),
			metrics.ResourceName(v1.ResourceMemory.String()): *info.Memory(),
		},
	}
}
